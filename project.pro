...
unix { 
    ...
    EXIV2IMAGEHPP = /usr/include/exiv2/image.hpp \
        /usr/local/include/exiv2/image.hpp \
        $$(LOCALSOFT)/include/exiv2/image.hpp
    for(path, EXIV2IMAGEHPP):exists($$path) { 
    ...
    # I think these are the only paths where we have to search for.
    # If your system is more exotic let me know.
    OPENEXRHEADER = /usr/include/OpenEXR/ImfHeader.h \
        /usr/local/include/OpenEXR/ImfHeader.h \
        /usr/local/include/ilmbase/ImfHeader.h \
        /usr/include/ilmbase/ImfHeader.h \
        $$(LOCALSOFT)/include/OpenEXR/ImfHeader.h
    for(path, OPENEXRHEADER):exists($$path) { 
    ...
    FFTW3HEADER = /usr/include/fftw3.h \
        ...
        $$(LOCALSOFT)/include/fftw3.h
    for(path, FFTW3HEADER):exists($$path) { 
    ...
    # I think these are the only paths where we have to search for.
    # If your system is more exotic let me know.
    LIBTIFFHEADER = /usr/include/tiffio.h \
        /usr/local/include/tiffio.h \
        $$(LOCALSOFT)/include/tiffio.h
    for(path, LIBTIFFHEADER):exists($$path) { 
    ...
    isEmpty(I18NDIR) { 
        mac:# I18NDIR=(QCoreApplication::applicationDirPath()+\"/i18n\")
        ...
        I18NDIR = QCoreApplication::applicationDirPath\\\(\\\)+"/../Resources/i18n"
        else:I18NDIR = $${PREFIX}/share/qtpfsgui/i18n
    }
    ...
    isEmpty(ENABLE_DEBUG)|contains(ENABLE_DEBUG, "no"):message ("Debug statements DISABLED")
    else { 
        DEFINES -= QT_NO_DEBUG_OUTPUT
    ...
    contains(MAJOR_MINOR_QT_VERSION,4.1):DEFINES += I18NDIR=\"$$I18NDIR\"
    
    # message("Detected Qt4.1")
    else:DEFINES += I18NDIR=\\\"$$I18NDIR\\\"
}
...
win32 { 
    ...
    LIBS += "-LC:\Program Files\GnuWin32\lib" \
        -llibtiff
