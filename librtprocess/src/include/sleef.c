...
__inline int xisinf(double x) { return x == librtprocess::RT_INFINITY || x == -librtprocess::RT_INFINITY; }
__inline int xisminf(double x) { return x == -librtprocess::RT_INFINITY; }
__inline int xispinf(double x) { return x == librtprocess::RT_INFINITY; }
...
__inline int xilogb(double d) {
  ...
  e = d == librtprocess::RT_INFINITY || d == -librtprocess::RT_INFINITY ? 2147483647 : e;
  return e;
}
...
__inline double xatan2(double y, double x) {
  ...
  r = mulsign(r, x);
  if (xisinf(x) || x == 0) r = librtprocess::RT_PI_2 - (xisinf(x) ? (sign(x) * (librtprocess::RT_PI_2)) : 0);
  if (xisinf(y)          ) r = librtprocess::RT_PI_2 - (xisinf(x) ? (sign(x) * (librtprocess::RT_PI*1/4)) : 0);
  if (             y == 0) r = (sign(x) == -1 ? librtprocess::RT_PI : 0);

  return xisnan(x) || xisnan(y) ? librtprocess::RT_NAN : mulsign(r, y);
}
...
__inline double xacos(double d) {
  return mulsign(atan2k(sqrt((1+d)*(1-d)), xfabs(d)), d) + (d < 0 ? librtprocess::RT_PI : 0);
}
...
__inline double xsin(double d) {
  ...
  q = (int)xrint(d * librtprocess::RT_1_PI);
...
__inline double xcos(double d) {
  ...
  q = 1 + 2*(int)xrint(d * librtprocess::RT_1_PI - 0.5);
...
__inline double2 xsincos(double d) {
  ...
  q = (int)xrint(d * (2 * librtprocess::RT_1_PI));
  ...
  if (xisinf(d)) { r.x = r.y = librtprocess::RT_NAN; }
...
__inline double xtan(double d) {
  ...
  if (xisinf(d)) u = librtprocess::RT_NAN;
...
__inline double xlog(double d) {
  ...
  if (xisinf(d)) x = librtprocess::RT_INFINITY;
  if (d < 0) x = librtprocess::RT_NAN;
  if (d == 0) x = -librtprocess::RT_INFINITY;
...
__inline double xpow(double x, double y) {
  ...
  result = xisnan(result) ? librtprocess::RT_INFINITY : result;
  result *=  (x >= 0 ? 1 : (!yisint ? librtprocess::RT_NAN : (yisodd ? -1 : 1)));

  double efx = mulsign(xfabs(x) - 1, y);
  if (xisinf(y)) result = efx < 0 ? 0.0 : (efx == 0 ? 1.0 : librtprocess::RT_INFINITY);
  if (xisinf(x) || x == 0) result = (yisodd ? sign(x) : 1) * ((x == 0 ? -y : y) < 0 ? 0 : librtprocess::RT_INFINITY);
  if (xisnan(x) || xisnan(y)) result = librtprocess::RT_NAN;
  if (y == 0 || x == 1) result = 1;
...
__inline double xsinh(double x) {
  ...
  y = xisinf(x) || xisnan(y) ? librtprocess::RT_INFINITY : y;
  y = mulsign(y, x);
  y = xisnan(x) ? librtprocess::RT_NAN : y;
...
__inline double xatanh(double x) {
  ...
  y = y > 1.0 ? librtprocess::RT_NAN : (y == 1.0 ? librtprocess::RT_INFINITY : (d.x + d.y) * 0.5);

  y = xisinf(x) || xisnan(y) ? librtprocess::RT_NAN : y;
  y = mulsign(y, x);
  y = xisnan(x) ? librtprocess::RT_NAN : y;
...
__inline double xsqrt(double d) { // max error : 0.5 ulp
  ...
  return d == librtprocess::RT_INFINITY ? librtprocess::RT_INFINITY : x * q;
}
...
__inline double xexp2(double a) {
  ...
  if (xispinf(a)) u = librtprocess::RT_INFINITY;
  if (xisminf(a)) u = 0;
  return u;
}
...
__inline double xexpm1(double a) {
  ...
  double x = d.x + d.y;
  if (xispinf(a)) x = librtprocess::RT_INFINITY;
  if (xisminf(a)) x = -1;
  return x;
}
...
__inline double xlog10(double a) {
  ...
  if (xisinf(a)) x = librtprocess::RT_INFINITY;
  if (a < 0) x = librtprocess::RT_NAN;
  if (a == 0) x = -librtprocess::RT_INFINITY;
...
__inline double xlog1p(double a) {
  ...
  if (xisinf(a)) x = librtprocess::RT_INFINITY;
  if (a < -1) x = librtprocess::RT_NAN;
  if (a == -1) x = -librtprocess::RT_INFINITY;
...
__inline int xisinff(float x) { return x == librtprocess::RT_INFINITY_F || x == -librtprocess::RT_INFINITY_F; }
__inline int xisminff(float x) { return x == -librtprocess::RT_INFINITY_F; }
__inline int xispinff(float x) { return x == librtprocess::RT_INFINITY_F; }
...
__inline float xsinf(float d) {
  ...
  q = rint(d * librtprocess::RT_1_PI_F);
...
__inline float xcosf(float d) {
  ...
  q = 1 + 2*rint(d * librtprocess::RT_1_PI_F - 0.5f);
...
__inline float2 xsincosf(float d) {
  ...
  q = rint(d * librtprocess::RT_2_PI_F);
  ...
  if (xisinff(d)) { r.x = r.y = librtprocess::RT_NAN_F; }
...
__inline float xtanf(float d) {
  ...
  q = rint(d * (float)(2 * librtprocess::RT_1_PI));
  ...
  if (xisinff(d)) u = librtprocess::RT_NAN_F;
...
__inline float atan2kf(float y, float x) {
  ...
  t = u * t;
  t = mlaf(t,s,s);
  return mlaf(q,(float)(librtprocess::RT_PI_F_2),t);
}
...
__inline float xatan2f(float y, float x) {
  ...
  r = mulsignf(r, x);
  if (xisinff(x) || x == 0) r = librtprocess::RT_PI_F/2 - (xisinff(x) ? (signf(x) * (float)(librtprocess::RT_PI_F*.5f)) : 0);
  if (xisinff(y)          ) r = librtprocess::RT_PI_F/2 - (xisinff(x) ? (signf(x) * (float)(librtprocess::RT_PI_F*.25f)) : 0);
  if (              y == 0) r = (signf(x) == -1 ? librtprocess::RT_PI_F : 0);

  return xisnanf(x) || xisnanf(y) ? librtprocess::RT_NAN_F : mulsignf(r, y);
}
...
__inline float xacosf(float d) {
  return mulsignf(atan2kf(sqrtf((1.0f+d)*(1.0f-d)), fabsf(d)), d) + (d < 0 ? (float)librtprocess::RT_PI : 0.0f);
}
...
__inline float xlogf(float d) {
  ...
  if (xisinff(d)) x = librtprocess::RT_INFINITY_F;
  if (d < 0) x = librtprocess::RT_NAN_F;
  if (d == 0) x = -librtprocess::RT_INFINITY_F;
