...
pfs::Frame* createHDR(const float* const arrayofexptime, const config_triple* const chosen_config, bool /*antighosting*/, int /*iterations*/, const bool ldrinput, ...)
{
  ...
  pfs::Array2D* Rr = Rj->getChannelData();
  pfs::Array2D* Gr = Gj->getChannelData();
  pfs::Array2D* Br = Bj->getChannelData();  
  ...
  switch( chosen_config->response_curve )
  {
    ...
    case FROM_ROBERTSON:
      ...
      if (ldrinput) {
        robertson02_getResponse(Rr, Gr, Br,
                                arrayofexptime, Ir, Ig, Ib, w, M, true, listldr);
      } else {
        robertson02_getResponse(Rr, Gr, Br,
                                arrayofexptime, Ir, Ig, Ib, w, M, false, listhdrR, listhdrG, listhdrB);
      }
      break;
  }
  ...
  switch( chosen_config->model ) {
    case ROBERTSON:
      ...
      if (chosen_config->response_curve==FROM_ROBERTSON)
        ...
        if (ldrinput) {
          robertson02_applyResponse(Rr, Gr, Br,
                                    arrayofexptime, Ir, Ig, Ib, w, M, true, listldr);
        } else {
          robertson02_applyResponse(Rr, Gr, Br,
                                    arrayofexptime, Ir, Ig, Ib, w, M, false, listhdrR, listhdrG, listhdrB);
    ...
    case DEBEVEC:
      ...
      // 	else {
      if (ldrinput)
        debevec_applyResponse(Rr, Gr, Br,
                              arrayofexptime, Ir, Ig, Ib, w, M, true, listldr);
      else
        debevec_applyResponse(Rr, Gr, Br,
                              arrayofexptime, Ir, Ig, Ib, w, M, false, listhdrR, listhdrG, listhdrB);
