...
struct TiffReaderData
{
    ...
    typedef boost::function<void (TiffReaderData*, Frame&, const TiffReaderParams&)> Callback;
    ...
    uint16 samplesPerPixel_;                    // number of channels in tiff file (only 1-3 are used)
    bool   hasAlpha_;
    ...
    void initReader()
    {
        ...
        /*static*/ Registry sm_registry = map_list_of
                ...
                (RegistryKey(PHOTOMETRIC_SEPARATED, 16),    boost::bind(&TiffReaderData::readCMYK<uint16_t>, _1, _2, _3))
                // (RegistryKey(PHOTOMETRIC_SEPARATED, 32), boost::bind(&TiffReaderData::readCMYK<float>, _1, _2, _3))
                (RegistryKey(PHOTOMETRIC_RGB, 8),           boost::bind(&TiffReaderData::readRGB<uint8_t>, _1, _2, _3))
                (RegistryKey(PHOTOMETRIC_RGB, 16),          boost::bind(&TiffReaderData::readRGB<uint16_t>, _1, _2, _3))
                (RegistryKey(PHOTOMETRIC_RGB, 32),          boost::bind(&TiffReaderData::readRGB<float>, _1, _2, _3))
                ;

        Registry::iterator it = sm_registry.find(RegistryKey(photometricType_, bitsPerSample_));
        if ( it == sm_registry.end() ) {
            throw pfs::io::InvalidHeader(std::string("TiffReader: cannot find the right function to parse the data ") +
                                         "bits per sample: " + boost::lexical_cast<std::string>(bitsPerSample_) +
                                         " photometric type: " + boost::lexical_cast<std::string>(photometricType_));
        }
...
private:
    cmsHTRANSFORM getColorSpaceTransform()
    {
        ...
        return cmsCreateTransform (hIn_.data(), cmsInputFormat,
                                   hsRGB_.data(), cmsOutputFormat,
                                   cmsIntent, 0);
    }
                         ...
                         const Converter& conv)
    {
        ...
        for (uint32 row = 0; row < height_; row++)
        {
            ...
            utils::transform(StrideIterator<InputDataType*>(tempBuffer.data(), samplesPerPixel_),
                             StrideIterator<InputDataType*>(tempBuffer.data() + width_*samplesPerPixel_, samplesPerPixel_),
                             StrideIterator<InputDataType*>(tempBuffer.data() + 1, samplesPerPixel_),
                             StrideIterator<InputDataType*>(tempBuffer.data() + 2, samplesPerPixel_),
                             Xc->row_begin(row),
                             Yc->row_begin(row),
                             Zc->row_begin(row),
                             conv);
                         ...
                         const Converter& conv)
    {
        ...
        for (uint32 row = 0; row < height_; row++)
        {
            ...
            utils::transform(StrideIterator<InputDataType*>(tempBuffer.data(), samplesPerPixel_),
                             ...
                             StrideIterator<InputDataType*>(tempBuffer.data() + 1, samplesPerPixel_),
                             StrideIterator<InputDataType*>(tempBuffer.data() + 2, samplesPerPixel_),
                             StrideIterator<InputDataType*>(tempBuffer.data() + 3, samplesPerPixel_),
                             Xc->row_begin(row),
                             Yc->row_begin(row),
                             Zc->row_begin(row),
                             conv);
    ...
    void readRGB(Frame & frame, const TiffReaderParams& params)
    {
        ...
        if ( xform ) {
            ...
            if ( hasAlpha_ ) {
                ...
                read3Components<InputDataType>(frame, params,
...
void TiffReader::close() {
    m_data.reset(new TiffReaderData);
}
...
void TiffReader::open()
{
    ...
    if ( !m_data->file_ ) {
        throw pfs::io::InvalidFile("TiffReader: cannot open file " + filename());
    }
    ...
    // check if planar... maybe in the future we can add support for tiled images?
    uint16 planarConfig;
    TIFFGetField(m_data->handle(), TIFFTAG_PLANARCONFIG, &planarConfig);
    if ( planarConfig != PLANARCONFIG_CONTIG ) {
        throw pfs::io::InvalidHeader("TiffReader: Unsopported planar configuration");
    }
    ...
    // photometric type
    if (!TIFFGetFieldDefaulted(m_data->handle(), TIFFTAG_PHOTOMETRIC, &m_data->photometricType_)) {
        throw pfs::io::InvalidHeader("TiffReader: unspecified photometric type");
    }
    ...
    // bits per sample
    if (!TIFFGetField(m_data->handle(), TIFFTAG_BITSPERSAMPLE, &m_data->bitsPerSample_) ) {
        throw pfs::io::InvalidHeader("TiffReader: unspecified bits per sample");
    }
    if ( m_data->bitsPerSample_ != 8 && m_data->bitsPerSample_ != 16 && m_data->bitsPerSample_ != 32) {
        throw pfs::io::InvalidHeader("TiffReader: invalid bits per sample (read: " +
                                     boost::lexical_cast<std::string>(m_data->bitsPerSample_) +
                                     ")");
    }

    // samples per pixel
    if (!TIFFGetField(m_data->handle(), TIFFTAG_SAMPLESPERPIXEL, &m_data->samplesPerPixel_) ) {
        throw pfs::io::InvalidHeader("TiffReader: unspecified samples per pixel");
    }
    ...
    case PHOTOMETRIC_LOGLUV: {
        if (m_data->compressionType_ != COMPRESSION_SGILOG &&
            ...
            throw pfs::io::InvalidHeader("TiffReader: only support SGILOG compressed LogLuv data");
        }
        TIFFSetField(m_data->handle(), TIFFTAG_SGILOGDATAFMT, SGILOGDATAFMT_FLOAT);
    } break;
    case PHOTOMETRIC_RGB: {
        ...
        if (colorSamples != 3) {
            throw pfs::io::InvalidHeader("TIFF: unsupported samples per pixel for RGB");
        }
    } break;
    case PHOTOMETRIC_SEPARATED: {
        if ( m_data->samplesPerPixel_ != 4 ) {
            throw pfs::io::InvalidHeader("TIFF: unsupported samples per pixel for CMYK");
        }
    } break;
    default: {
        throw pfs::io::InvalidHeader("TiffReader: unsupported photometric type");
    } break;
    }
