...
// multithreaded and vectorized by Ingo Weyrich
    ...
    float **redFactor = nullptr;
    float **blueFactor = nullptr;
    float **oldraw = nullptr;
    if (avoidColourshift) {
        redFactor = (float**)malloc((H+1)/2 * sizeof(redFactor));
        blueFactor = (float**)malloc((H+1)/2 * sizeof(blueFactor));
        oldraw = (float**)malloc(H * sizeof(blueFactor));
        ...
        redFactor[0] = (float*)malloc((W+1)/2 * (H+1)/2 * sizeof(*redFactor[0]));
        blueFactor[0] = (float*)malloc((W+1)/2 * (H+1)/2 * sizeof(*blueFactor[0]));
        oldraw[0] = (float*)malloc((W+1)/2 * H * sizeof(*oldraw[0]));
        if (!redFactor[0] || !blueFactor[0] || !oldraw[0]) {
            if (redFactor[0]) {
                free(redFactor[0]);
            }
            if (blueFactor[0]) {
        ...
        for (int i = 1; i < H; ++i) {
            oldraw[i] = oldraw[i - 1] + (W+1)/2;
        }
        for (int i = 1; i < (H + 1) / 2; ++i) {
            redFactor[i] = redFactor[i - 1] + (W+1)/2;
            blueFactor[i] = blueFactor[i - 1] + (W+1)/2;
        }
            ...
            if (avoidColourshift && !rc) {
                ...
                #pragma omp parallel
                {
                    ...
                    for (int i = winy; i < winh; ++i) {
                        ...
                        float **nonGreen = colour == 0 ? redFactor : blueFactor;
                        int j = firstCol;
    #ifdef __SSE2__
                        for (; j < winw - 7; j += 8) {
                    ...
                    #pragma omp single
                    {
                        if (H % 2) {
                            ...
                            float **nonGreen = colour == 0 ? blueFactor : redFactor;
