...
float apply_arctg_slope10(float Ip, float I, float I2, float I3, float I4,
...
void producto(fftwf_complex *A, fftwf_complex *B, fftwf_complex *AB, int fil,
              int col) {
    //  int length=fil*(col/2+1);
    // fftw_complex* C= (fftw_complex*) fftw_malloc(sizeof(fftw_complex) *
    // length);
    int length = fil * col;
...
#pragma omp parallel for
    for (int i = 0; i < fil; i++)
        for (int j = 0; j < col; j++)
            res[i * col + j] = normaliza * exp(-((i - mitfil) * (i - mitfil) +
                                                 (j - mitcol) * (j - mitcol)) /
                                               (2 * sigma * sigma));
}
...
#pragma omp parallel for
    ...
    fftwf_plan pinvU2 =
        fftwf_plan_dft_c2r_2d(fil, col, U2G, iu2, FFTW_ESTIMATE);

    float *iu3 = fftwf_alloc_real(length);
    fftwf_plan pinvU3 =
        fftwf_plan_dft_c2r_2d(fil, col, U3G, iu3, FFTW_ESTIMATE);

    float *iu4 = fftwf_alloc_real(length);
    fftwf_plan pinvU4 =
        fftwf_plan_dft_c2r_2d(fil, col, U4G, iu4, FFTW_ESTIMATE);

    float *iu5 = fftwf_alloc_real(length);
    fftwf_plan pinvU5 =
        fftwf_plan_dft_c2r_2d(fil, col, U5G, iu5, FFTW_ESTIMATE);

    float *iu6 = fftwf_alloc_real(length);
    fftwf_plan pinvU6 =
        fftwf_plan_dft_c2r_2d(fil, col, U6G, iu6, FFTW_ESTIMATE);

    float *iu7 = fftwf_alloc_real(length);
    fftwf_plan pinvU7 =
        fftwf_plan_dft_c2r_2d(fil, col, U7G, iu7, FFTW_ESTIMATE);
    ...
    while (difference > threshold_diff) {
        ...
        for (int color = 0; color < colors; color++) {
            ...
            vsmul(iu2, norm, iu2, length);

            fftwf_execute(pinvU3);
            vsmul(iu3, norm, iu3, length);

            fftwf_execute(pinvU4);
            vsmul(iu4, norm, iu4, length);

            fftwf_execute(pinvU5);
            vsmul(iu5, norm, iu5, length);

            fftwf_execute(pinvU6);
            vsmul(iu6, norm, iu6, length);

            fftwf_execute(pinvU7);
            vsmul(iu7, norm, iu7, length);

#pragma omp parallel for
            for (int i = 0; i < length; i++) {
                // compute contrast component
                u0[i] = apply_arctg_slope10(u0[i], iu[i], iu2[i], iu3[i],
                                            iu4[i], iu5[i], iu6[i], iu7[i]);
            ...
            float norm1 =
                (1.0 +
                 dt * (1.0 + 255.0 / 253.0));  // assuming alpha=255/253,beta=1
#pragma omp parallel for
            for (int i = 0; i < length; i++) {
                RGB[color][i] = (RGB[color][i] +
                                 dt * (RGBorig[color][i] + 0.5 * u0[i] +
                                       255.0 / 253.0 * med[color])) /
                                norm1;
    ...
    ph.setValue(90);
#pragma omp parallel for
    for (int c = 0; c < 3; c++) escala(RGB[c], length, 1.f, 0.f);
